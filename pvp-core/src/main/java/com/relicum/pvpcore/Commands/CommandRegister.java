...
/**
 * CommandRegister used to register commands and also pre filter inbound
 * commands.
 * <p>
 * Create a new instance of {@link #CommandRegister(org.bukkit.plugin.Plugin)}
 * in your main plugin class.
 * <p>
 * Create it never the top of the onLoad. Then register all your commands using
 * the {@link #register(com.relicum.pvpcore.Commands.AbstractCommand)} .
 * <p>
 * To create commands see {@link AbstractCommand}
 *
 * @author Relicum
 * @version 0.0.1
 */
public class CommandRegister implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!commands.containsKey(cmd.getName().toLowerCase()) && !isRootCmd(cmd.getName().toLowerCase()))
        { // Check me can find details of the command

            sender.sendMessage(ChatColor.RED + "Error command not found");
            return true;
        }

        if (rootCmd.contains(cmd.getName().toLowerCase()) && args.length == 0)
        { // Deal with main sub command

            sender.sendMessage(ChatColor.AQUA + "Main Cmd help section");
            return true;
        }

        if ((rootCmd.contains(cmd.getName().toLowerCase()) && args.length == 1) && !commands.containsKey(args[0].toLowerCase()))
        { // Deal with main sub command
        ...
        if ((sub.isSub() && args.length >= 1) || !sub.isSub())
        {
            ...
            { // Check if console can receive command
            ...
            if (sub.getMax() != -1 && (total > sub.getMax()))
            ...
            if (sub.getMax() != -1 && (sub.getMin() > sub.getMax()))
            { // Check I haven't made a mistake and got args round the wrong way
        ...
        plugin.getLogger().severe("Unhandled command Exception needs investigating");
     ...
     *
     * @param <T> extends {@link AbstractCommand}
    ...
    protected boolean registerCommand(String name, AbstractCommand cmd) {
        ...
        if (cmp.register(lab, "mc", cd))
        {
            ...
            // System.out.println("Command: /" + cd.getLabel() +
     ...
     * <p>
     * It creates Immutable lists and Maps to help of sync and efficiency.
     */
